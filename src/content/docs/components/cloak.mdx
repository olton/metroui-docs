...
It is recommended to use the `cloak` class for items that you need to hide when downloading a page. 
When the item is ready to display, add the class `remove-cloak` to animate its appearance. 
Use the `add-cloak` class to hide the visible element.

If class `cloak` is applied to `body`, Metro UI will delete it automatically on completed page loading.
...
| Class | Description |
| ----- | ----------- |
| `cloak` | Makes an element invisible (opacity: 0) |
