...
By default, the Dropdown component opens the contents with the automatic selection of the container's position according to the current duty.
But in some cases, you need to open the container on the appropriate side. 
You can do this with an attribute `data-open-mode`. 
Affordable values: `auto` (by default), `up`, `down`, `left`, and `right`.

```html
<!-- Dropdown that opens upward -->
<div class="pos-relative">
    <button class="dropdown-toggle">Click me</button>
    <ul data-role="dropdown" class="d-menu" data-open-mode="up">
...
Іноді потрібно залишити контейнер відкритим за будь яких обставин. 
Для цього існує 2 шляхи: 

1. Додати класс `keep-open`, 
2. Використовувати атрибут `data-no-close="true"` 

```html
<!-- Dropdown that stays open -->
<div class="dropdown-toggle">
    Click me
    <div class="dropdown" data-role="dropdown" data-no-close="true">
...
```javascript
// Global configuration, you must create this object before loading metro.js
globalThis.metroDropdownSetup = {
    noClose: true,
    openMode: "up"
};
