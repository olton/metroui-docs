...
Using this class, you can create truly responsive sites on the Metro UI.
...
Also, you can use additional methods to manipulate arrays.
...
const devTools = model.runDevTools({
    enabled: true,
    timeTravel: true,
    maxSnapshots: 50
});
...
model.dom.validateModel();
model.store.isValidPath('name'); // true
model.store.isValidPath('lastname'); // false
```
