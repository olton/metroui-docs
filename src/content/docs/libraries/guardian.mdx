...
<div>
    <SourceButton href="https://github.com/olton/guardian" icon="github" text="Repository"/>
    <SourceButton href="https://www.npmjs.com/package/@olton/guardian" icon="seti:npm" text="Package"/>
</div> 
...
Check if the value is between two numbers: `between(min, max, err_message)`.
...
Check if the value is a credit card: `creditCard(err_message)`.
...
Check if the value is a date: `date(err_message)`.
...
Check if the value is a digits: `digits(err_message)`.
...
Check if the value is a hex color: `hexColor(err_message)`.
...
Check if the value has a specified length: `length(min, max, err_message)`.

```javascript
const schema = G.length(1, 10)
```
